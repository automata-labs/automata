...
describe('AToken', async () => {
  ...
  describe('#mint', async () => {
    it('should mint', async () => {
      await join(wallet, aToken.address, null, e18(100));
      expect((await read(token.address, aToken.address)).x).to.equal(e18(100));
      ...
      expect(await aToken.totalSupply()).to.equal(e18(100));
      expect(await aToken.balanceOf(wallet.address)).to.equal(e18(100));
  ...
  describe('#burn', async () => {
    it('should burn', async () => {
      ...
      // burn & join
      await aToken.transfer(aToken.address, e18(100));
    ...
    it('should emit an event', async () => {
      ...
      await expect(aToken.burn(wallet.address))
        .to.emit(aToken, 'Transfer')
        .withArgs(aToken.address, ethers.constants.AddressZero, e18(100));
    });
  });
});
